...
export function ProjectIcon({ name, size = 200 }: { name: string; size?: number }) {
  ...
  if (lower.includes("mouzo") || lower.includes("nemy") || lower.includes("delivery")) {
    const delGrad = `${uid}-delGrad`;
    const delShadow = `${uid}-delShadow`;
    return (
      <svg width={size} height={size} viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <g transform="translate(30, 60)" filter={`url(#${delShadow})`}>
          <path d="M65 10h100c5 0 10 5 10 10v45c0 5-5 10-10 10H65c-5 0-10-5-10-10V20c0-5 5-10 10-10z" fill={`url(#${delGrad})`} opacity="0.9" />
          ...
          <path d="M10 25c0-8 7-15 15-15h35l10 25H10V25z" fill="#6C5CE7" opacity="0.9" />
  ...
  if (lower.includes("sistema") || lower.includes("erp") || lower.includes("start")) {
    const erpGrad = `${uid}-erpGrad`;
    const erpBar = `${uid}-erpBar`;
    const erpShadow = `${uid}-erpShadow`;
    return (
      <svg width={size} height={size} viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
        ...
        <g transform="translate(35, 25)" filter={`url(#${erpShadow})`}>
          <path d="M0 20c0-11 9-20 20-20h130c11 0 20 9 20 20v130c0 11-9 20-20 20H20c-11 0-20-9-20-20V20z" fill={`url(#${erpGrad})`} fillOpacity="0.15" />
  ...
  if (lower.includes("prestamo") || lower.includes("finance") || lower.includes("pago")) {
    const finGrad = `${uid}-finGrad`;
    const finShadow = `${uid}-finShadow`;
    return (
      <svg width={size} height={size} viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
        ...
        <g transform="translate(45, 25)" filter={`url(#${finShadow})`}>
          <path d="M15 40c0-8 7-15 15-15h100c8 0 15 7 15 15v80c0 8-7 15-15 15H30c-8 0-15-7-15-15V40z" fill={`url(#${finGrad})`} fillOpacity="0.12" rx="12" />
          <rect x="25" y="50" width="20" height="14" rx="3" fill={`url(#${finGrad})`} opacity="0.5" />
          <text x="75" y="85" textAnchor="middle" fill={`url(#${finGrad})`} fontSize="42" fontWeight="700" fontFamily="system-ui, sans-serif" opacity="0.9">$</text>
          ...
          <text x="150" y="46" textAnchor="middle" fill="white" fontSize="12" fontWeight="700" opacity="0.8">$</text>
          ...
          <text x="155" y="105" textAnchor="middle" fill="white" fontSize="9" fontWeight="700" opacity="0.7">$</text>
        </g>
      </svg>
    );
  }
  ...
  if (lower.includes("click") || lower.includes("publik") || lower.includes("marketing")) {
    const mktGrad = `${uid}-mktGrad`;
    const mktShadow = `${uid}-mktShadow`;
    return (
      <svg width={size} height={size} viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
        ...
        <g transform="translate(40, 30)" filter={`url(#${mktShadow})`}>
          <path d="M90 20L150 5v50l-60-15M30 40v20c0 5 5 10 10 10h10l40 10V30L50 20H40c-5 0-10 5-10 10z" fill={`url(#${mktGrad})`} opacity="0.8" />
          <path d="M15 35 Q25 50 15 65" stroke={`url(#${mktGrad})`} strokeWidth="3" fill="none" opacity="0.6" strokeLinecap="round">
            <animate attributeName="opacity" values="0.6;0.2;0.6" dur="1.5s" repeatCount="indefinite" />
          </path>
          <path d="M5 30 Q20 50 5 70" stroke={`url(#${mktGrad})`} strokeWidth="2" fill="none" opacity="0.4" strokeLinecap="round">
            <animate attributeName="opacity" values="0.4;0.1;0.4" dur="2s" repeatCount="indefinite" />
          </path>
          <rect x="20" y="90" width="130" height="70" rx="8" fill={`url(#${mktGrad})`} fillOpacity="0.08" />
          <path d="M30 150L60 120L85 130L110 90L130 60L145 75" stroke={`url(#${mktGrad})`} strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" />
  ...
  // Default: code / dev
  const codeGrad = `${uid}-codeGrad`;
  const codeShadow = `${uid}-codeShadow`;
  return (
    <svg width={size} height={size} viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
      ...
      <g transform="translate(30, 30)" filter={`url(#${codeShadow})`}>
        <path d="M0 0h180v140H0z" fill="white" fillOpacity="0.04" rx="12" />
        ...
        <text x="20" y="55" fill="#FF007F" fontSize="14" fontFamily="monospace" opacity="0.8">import</text>
        <text x="70" y="55" fill="white" fontSize="14" fontFamily="monospace" opacity="0.9">{" { "}</text>
        <text x="20" y="75" fill="#6C5CE7" fontSize="14" fontFamily="monospace" opacity="0.8">  Caskiuz</text>
        <text x="145" y="55" fill="white" fontSize="14" fontFamily="monospace" opacity="0.9">{" }"}</text>
        <text x="20" y="95" fill="#00E5FF" fontSize="14" fontFamily="monospace" opacity="0.8">  from</text>
        <text x="20" y="115" fill="#8888A0" fontSize="14" fontFamily="monospace" opacity="0.5">    'caskiuz.dev'</text>
